...
class WanVideoLoraSelect:
    ...
    def INPUT_TYPES(s):
        return {
            ...
            "optional": {
                ...
                "auto_balance": ("BOOLEAN", {"default": False, "tooltip": "Automatically balance LoRA strengths so their sum does not exceed a limit"}),
                "max_total_strength": ("FLOAT", {"default": 1.0, "min": 0.1, "max": 10.0, "step": 0.1, "tooltip": "Maximum total strength for all LoRAs when auto_balance is enabled"}),
            },
            "hidden": {
                "unique_id": "UNIQUE_ID",
            },
    ...
    def getlorapath(self, lora, strength, unique_id, blocks={}, prev_lora=None, low_mem_load=False, merge_loras=True, auto_balance=False, max_total_strength=1.0):
        if not merge_loras:
        ...
        if auto_balance and len(loras_list) > 0:
            total_strength = sum(abs(l["strength"]) for l in loras_list)
            if total_strength > max_total_strength:
                scale = max_total_strength / total_strength
                log.info(f"Auto-balancing LoRAs: total strength {total_strength:.4f} exceeds max {max_total_strength:.4f}. Scaling by {scale:.4f}")
                for l in loras_list:
                    l["strength"] = round(l["strength"] * scale, 4)
...
class WanVideoLoraSelectMulti:
    ...
    def getlorapath(self, lora_0, strength_0, lora_1, strength_1, lora_2, strength_2,
                ...
                low_mem_load=False, merge_loras=True, auto_balance=False, max_total_strength=1.0):
        if not merge_loras:
            low_mem_load = False  # Unmerged LoRAs don't need low_mem_load
        ...
        if auto_balance and len(loras_list) > 0:
            ...
            if total_strength > max_total_strength:
                scale = max_total_strength / total_strength
                log.info(f"Auto-balancing Multi-LoRAs: total strength {total_strength:.4f} exceeds max {max_total_strength:.4f}. Scaling by {scale:.4f}")
                for l in loras_list:
                    l["strength"] = round(l["strength"] * scale, 4)
