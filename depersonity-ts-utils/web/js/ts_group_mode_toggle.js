...
const NODE_TYPE   = "Depersonity_TSGroupModeToggle";
const LEGACY_NODE_TYPE = "TSGroupModeToggle";
const TITLE       = "Depersonity Group Mode Toggle";
const CATEGORY    = "Depersonity";
const WIDGET_H    = 20;
const MARGIN      = 15;
const NAV_AREA    = 28;
const SETTINGS_H  = 24;

function isToggleNodeType(nodeType) {
    return nodeType === NODE_TYPE || nodeType === LEGACY_NODE_TYPE;
}
...
function startPeriodicRefresh() {
    ...
    _refreshInterval = setInterval(() => {
        ...
        for (const node of graph._nodes || []) {
            if (!isToggleNodeType(node.type)) continue;
            node.type = NODE_TYPE;
...
app.registerExtension({
    ...
    loadedGraphNode(node) {
        if (!isToggleNodeType(node.type)) return;
        node.type = NODE_TYPE;
        node.comfyClass = NODE_TYPE;
